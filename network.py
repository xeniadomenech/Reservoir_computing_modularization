...
class Network():
    ...
    def link_adjacency_matrix(self, d, k, states, rc_premotor, rc_motor, reser, inpu):
            
            L = len(rc_premotor.Network.W)
            M = len(rc_motor.Network.W)
            Lj = rc_premotor.Network.K
            Mj = rc_motor.Network.K
            ...
            self.W_in = np.zeros([M+L,Lj+Mj]) #input matrix
    
            for i in range(L):
                #Copying the information of premotor's matrix 
                for j in range(Lj):
                    self.W_in[i,j] = rc_premotor.Network.W_in[i,j]
                #Generating new information
                for j in range(Lj,Mj+Lj):
                    p = np.random.uniform()
                    if 0 <= p <= inpu:
                        self.W_in[i,j] = np.random.normal(loc=0, scale=1, size=1)[0]         
            ...
            for i in range(L,M+L):
                ...
                for j in range(Mj):
                    ...
                    if 0 <= p <= inpu:
                        ...
                        self.W_in[i,j] = 0
                #Copying the information of motor's matrix 
                for j in range(Lj,Mj+Lj):
                    self.W_in[i,j] = rc_motor.Network.W_in[i-L,j-Lj]
            ...
            self.W = np.zeros([M+L, M+L])
            
            for i in range(L):
                #Copying the information of premotor's matrix 
                for j in range(L):
                    self.W[i,j] = rc_premotor.Network.W[i,j]
                #Generating new information
            ...
            for i in range(L,M+L):
                ...
                #Copying the information of premotor's matrix 
                for j in range(L,M+L):
                    self.W[i,j] = rc_motor.Network.W[i-L,j-L]
    ...
    def compute_nodes_trajectories(self,num_columns, num_trials, test=False, t_autonom=None): 
        """
        If test=False:
            -It computes self.trajectories, which is a matrix of size TxN, where each column is 
            the trajectory of a node. Notice that the first row corresponds to the initial state
